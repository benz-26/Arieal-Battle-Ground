...
public class RaycastWeapon : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        audioSource.spatialBlend = 1f; // Make the sound 3D (adjust volume based on distance)
        audioSource.minDistance = 1f; // Minimum distance at which the sound is audible
        audioSource.maxDistance = 50f; // Maximum distance at which the sound can be heard
    ...
    public void StartFiring() {
        // Prevent firing when there are no bullets and no clips
        if (ShouldReload())
        {
            return;
        }
    ...
    {
        return ammoCount == 0 && clipCount == 0 && clipSize > 0;
    }
    ...
    {
        ammoCount = clipSize;
        clipCount = Mathf.Max(clipCount - 1, 0); // Prevent clipCount from going below 0
    }
