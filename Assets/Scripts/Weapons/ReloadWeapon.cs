...
public class ReloadWeapon : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // Check if the weapon should reload and prevent reloading if both ammo and clip are 0
            if ((Input.GetKeyDown(KeyCode.R) || weapon.ShouldReload()) && !weapon.IsLowAmmo())
            {
                if (weapon.clipSize > 0 && weapon.clipCount > 0)
                {
                    isReloading = true;
